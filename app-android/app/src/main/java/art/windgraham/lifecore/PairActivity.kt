...
class PairActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Api.init(this)
        // v0.2：启动时检查最近崩溃（如有则弹一次性 dialog，让用户主动查看/忽略/清除）
        maybeShowLastCrash()
    ...
    private fun maybeShowLastCrash() {
        ...
        crashShown = true
        val last = CrashHandler.peekLast(this) ?: return
        val ts = DateFormat.getMediumDateFormat(this).format(Date(last.ts)) + " " +
                 DateFormat.getTimeFormat(this).format(Date(last.ts))
        AlertDialog.Builder(this)
            .setTitle(R.string.crash_dialog_title)
            .setMessage(getString(R.string.crash_dialog_msg, ts, last.fileName))
            .setPositiveButton(R.string.crash_dialog_view) { _, _ ->
                AlertDialog.Builder(this)
                    .setTitle(last.fileName)
                    .setMessage(last.summary)
                    .setPositiveButton(android.R.string.ok, null)
                    .show()
            }
            .setNeutralButton(R.string.crash_dialog_clear) { _, _ ->
                CrashHandler.clearLast(this)
                toast("已清除崩溃记录")
            }
